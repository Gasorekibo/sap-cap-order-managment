...
export default async function (srv) {
   ...
   srv.before("CREATE", Products, async (req:Request) => {
      ...
      const existingProduct = await srv.tx(req).run(
        SELECT.one.from(Products).where({ name: req.data.name })
      );
      if (existingProduct) {
   ...
   srv.before("CREATE", Customers, async (req:Request) => {
      ...
      const existingCustomer = await srv.tx(req).run(
        SELECT.one.from(Customers).where({ email: req.data.email })
      );
      if (existingCustomer) {
   ...
   srv.before("CREATE", Orders, async (req: Request) => {
    ...
    const customer = await srv.tx(req).run(
      SELECT.one.from(Customers).where({ ID: req.data.customer_ID })
    );
    ...
    try {
      const productPromises = req.data.items.map(async (orderItem: any) => {
        ...
        const product = await srv.tx(req).run(
          SELECT.one.from(Products).where({ ID: orderItem.product_ID })
        );
